...
def test_relation_families_accept_claim_preserving_synonyms() -> None:
    cases = (
        ...
        ("Did the authors compare both systems?", "Our system outperforms the baseline."),
    ...
    for question, text in cases:
        trace = boolean_proposition_binding_trace(question, "yes", [_item("support", text)])
...
def test_actor_and_scope_mismatch_are_rejected_with_trace() -> None:
    ...
    assert trace["rejected_candidates"][0]["reason"].startswith("excluded_")
