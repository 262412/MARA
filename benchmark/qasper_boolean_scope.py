...
def _matching_item(
    ...
    for item in items:
        text = _normalized(_item_text(item))
...
def _actor(quote: str, section_role: str) -> str:
    ...
    if (
        ...
        return "cited_work"
    if re.search(
        r"\b(?:we|our|this (?:paper|article|study|work)|the authors)\b",
        lowered,
    ):
...
def _section_role(item: dict[str, Any], quote: str) -> str:
    ...
    if re.search(r"\b(?:related work|background|previous work|prior work)\b", lowered):
        return "related_work"
    if re.search(
        r"\b(?:experiments?|evaluation|evaluate|datasets?|corpora|corpus)\b",
        lowered,
    ):
        return "experiments"
    if re.search(r"\b(?:result|finding|performance)\b", lowered):
        return "results"
    if re.search(r"\b(?:method|approach|procedure|annotation)\b", lowered):
        return "methods"
