...
def terminal_outcome_record(prediction: dict[str, Any]) -> dict[str, Any]:
    ...
    state_commit = (
        state.get("terminal_semantic_commit") if isinstance(state, dict) else None
    )
    commit = state_commit if applicable else _first_commit_alias(prediction)
    projection_present = terminal_commit_projection_present(commit)
    alias_consistent = _commit_aliases_match(
        prediction,
        commit,
        require_all=applicable,
    )
    ...
    record = {
        ...
        "contract_violation": bool(
            applicable and (not projection_present or not alias_consistent)
        ),
        "outcome": outcome,
        "reason": reason,
    }
...
def _first_commit_alias(prediction: dict[str, Any]) -> Any:
    return prediction.get("engine_terminal_commit") or prediction.get(
        "terminal_semantic_commit"
    )


def _commit_aliases_match(
    ...
    *,
    ...
    if require_all:
        return bool(
            ...
            and commit
            and all(isinstance(alias, dict) and alias == commit for alias in aliases)
        )
    populated = [alias for alias in aliases if isinstance(alias, dict) and alias]
    return not populated or all(alias == commit for alias in populated)
