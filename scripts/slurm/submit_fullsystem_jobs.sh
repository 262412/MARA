...
printf 'wave_index\tbarrier_job_id\tdependency\truntime_list\n' >"$BARRIER_TABLE"
...
submit_wave_barrier() {
  ...
  barrier_id="$(sbatch --parsable \
    ...
    --export="ALL,MARA_RUNTIME_DIR_LIST=${runtime_list}" \
    "$BARRIER_SCRIPT")"
  barrier_id="${barrier_id%%;*}"
  printf '%s\t%s\t%s\t%s\n' "$WAVE_INDEX" "$barrier_id" "$dependency" "$runtime_list" >>"$BARRIER_TABLE"
...
submit_job() {
  ...
  if [[ "$kind" == "text" ]]; then
    job_id="$(sbatch --parsable $dependency_args \
      ...
      --export="ALL,MARA_PROJECT_ROOT=${PROJECT_ROOT},MARA_HPC_HOME=/mnt/scratch/users/tbczhang/mara-hpc,MARA_TEXT_RUN_ROOT=${RUN_ROOT},MARA_TEXT_OUTPUT_DIR=${output_root},MARA_TEXT_HEALTH_DIR=${RUN_ROOT}/00_health,MARA_TEXT_MANIFEST=${manifest},MARA_TEXT_ROUTE=${route},MARA_TEXT_LIMIT=${limit},MARA_TEXT_SAMPLE_SEED=${SEED},MARA_TEXT_MAX_CONTEXT_LENGTH=3000,MARA_TEXT_ROUTE_TIMEOUT_SECONDS=${timeout},MARA_TEXT_ARTIFACT_DETAIL=compact,MARA_REQUIRE_CONTRACT_SMOKE=0,MARA_SEMANTIC_EVALUATOR=off,MARA_TEXT_SUITE_NAME=${suite},MARA_TEXT_NUM_SHARDS=${num_shards},MARA_TEXT_SHARD_INDEX=${shard},MARA_EXECUTION_PLAN=${PLAN_PATH},MARA_EXECUTION_JOB_CONTRACT=${contract_path},MARA_EXECUTION_JOB_KEY=${suite},MARA_BENCHMARK_RUNTIME_ROOT=${RUNTIME_ROOT},MARA_TEXT_GPU=0,MARA_RETRIEVAL_GPU=1,MARA_RETRIEVAL_DEVICES=cuda,UV_NO_CACHE=1" \
    ...
    job_id="$(sbatch --parsable $dependency_args \
      ...
      --output="${RUN_ROOT}/slurm/%x-%j.out" \
      --error="${RUN_ROOT}/slurm/%x-%j.err" \
      --export="ALL,MARA_PROJECT_ROOT=${PROJECT_ROOT},MARA_HPC_HOME=/mnt/scratch/users/tbczhang/mara-hpc,MARA_BENCHMARK_RUNTIME_ROOT=${RUNTIME_ROOT},MARA_MULTIMODAL_OUTPUT_DIR=${output_root},MARA_MULTIMODAL_MANIFEST=${manifest},MARA_MULTIMODAL_ROUTE=${route},MARA_MULTIMODAL_LIMIT=${limit},MARA_MULTIMODAL_SAMPLE_SEED=${SEED},MARA_MULTIMODAL_ROUTE_TIMEOUT_SECONDS=${timeout},MARA_MULTIMODAL_SUITE_NAME=${suite},MARA_MULTIMODAL_NUM_SHARDS=${num_shards},MARA_MULTIMODAL_SHARD_INDEX=${shard},MARA_EXECUTION_PLAN=${PLAN_PATH},MARA_EXECUTION_JOB_CONTRACT=${contract_path},MARA_EXECUTION_JOB_KEY=${suite},MARA_RETRIEVAL_DEVICES=cuda,MARA_VLM_SERVE_SCRIPT=serve_qwen3_vl_8b_4k.sh,MARA_VLM_MAX_MODEL_LEN=8192,MARA_VLM_GPU_MEMORY_UTILIZATION=0.70,MARA_VLM_MAX_IMAGES=1,MARA_VLM_MAX_OUTPUT_TOKENS=96,MARA_VLM_TIMEOUT=180,MARA_VLM_EVIDENCE_TEXT_CHARS=120,MARA_PAGE_IMAGE_RANK_CANDIDATE_LIMIT=48,MARA_COLVISION_DEVICE=cuda:0,MARA_COLVISION_LOCAL_FILES_ONLY=1,UV_NO_CACHE=1" \
  ...
  local runtime_dir="${RUNTIME_ROOT}/${suite}/job${job_id}"
  WAVE_JOB_IDS+=("$job_id")
