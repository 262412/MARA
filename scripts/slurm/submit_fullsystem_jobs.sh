...
submit_job() {
  ...
  if [[ "$kind" == "text" ]]; then
    job_id="$(sbatch --parsable $dependency_args \
      ...
      --export="ALL,MARA_PROJECT_ROOT=${PROJECT_ROOT},MARA_HPC_HOME=/mnt/scratch/users/tbczhang/mara-hpc,MARA_TEXT_RUN_ROOT=${RUN_ROOT},MARA_TEXT_OUTPUT_DIR=${output_root},MARA_TEXT_HEALTH_DIR=${RUN_ROOT}/00_health,MARA_TEXT_MANIFEST=${manifest},MARA_TEXT_ROUTE=${route},MARA_TEXT_LIMIT=${limit},MARA_TEXT_SAMPLE_SEED=${SEED},MARA_TEXT_MAX_CONTEXT_LENGTH=3000,MARA_TEXT_ROUTE_TIMEOUT_SECONDS=${timeout},MARA_TEXT_ARTIFACT_DETAIL=compact,MARA_REQUIRE_CONTRACT_SMOKE=0,MARA_SEMANTIC_EVALUATOR=off,MARA_TEXT_SUITE_NAME=${suite},MARA_TEXT_NUM_SHARDS=${num_shards},MARA_TEXT_SHARD_INDEX=${shard},MARA_EXECUTION_PLAN=${PLAN_PATH},MARA_EXECUTION_TABLE=${JOB_TABLE},MARA_EXECUTION_JOB_CONTRACT=${contract_path},MARA_EXECUTION_JOB_KEY=${suite},MARA_BENCHMARK_RUNTIME_ROOT=${RUNTIME_ROOT},MARA_TEXT_GPU=0,MARA_RETRIEVAL_GPU=1,MARA_RETRIEVAL_DEVICES=cuda,UV_NO_CACHE=1" \
