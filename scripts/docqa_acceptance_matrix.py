...
for candidate in (REPO_ROOT, REPO_ROOT / "libs" / "ktem", REPO_ROOT / "libs" / "kotaemon"):
...
class AcceptanceMatrix:
    def __init__(self, keep_artifacts: bool = False, verbose: bool = False):
        ...
        self.env["PYTHONPATH"] = (
            f"{self.repo_root / 'libs' / 'ktem'};{self.repo_root / 'libs' / 'kotaemon'}"
        )
    ...
    def _resolve_cli_executable(self) -> list[str]:
        exe = self.repo_root / ".venv" / "Scripts" / "kotaemon.exe"
        if exe.exists():
            return [str(exe)]
        python_exe = self.repo_root / ".venv" / "Scripts" / "python.exe"
        return [str(python_exe), "-m", "kotaemon.cli"]
