...
def offline(event, args):
    if event in ('socket.connect', 'socket.getaddrinfo'):
        raise RuntimeError('Installed evidence binding attempted network access')
...
plan = with_plan_id(QueryPlan('extractive', 'lookup', evidence_slots=(slot,)), 'topic')
...
continued, trace = query_evidence_binding.bind_evidence_slots_monotonic(bound, [record])
