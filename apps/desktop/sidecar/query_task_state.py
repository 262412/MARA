...
def task_from_dict(item: dict[str, Any]) -> QueryTaskState:
    terminal_commit, terminal_outcome, terminal_outcome_reason = _terminal_projection(
        item
    )
...
def _terminal_projection(
    ...
    if value in (None, {}):
        commit: dict[str, Any] = {}
    elif isinstance(value, dict) and terminal_commit_projection_present(value):
        commit = deepcopy(value)
    else:
        raise ValueError("Query task terminal semantic commit is invalid.")
    outcome = terminal_commit_outcome(commit)
    reason = str(commit.get("outcome_reason") or "")
    aliases = {
        "terminal_outcome": outcome,
        "terminal_outcome_reason": reason,
    }
    for key, expected in aliases.items():
        if key in item and str(item.get(key) or "") != expected:
            raise ValueError(f"Query task {key} does not match its terminal commit.")
    return commit, outcome, reason
