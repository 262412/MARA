...
function fixture() {
  ...
  return {expected: {id: 'uuid-B', user: 'owner', name: 'B', messages: data[3], sources: ['same-file'], retrievalMessages: ['B evidence'], filename: 'exact.txt'},
    ...
    framework: {errors: [], records: [{phase: 'installed'},
      ...
      {phase: 'logpoints_state', sequence: 1, active: true}]},
...
const negatives = {
  ...
  'completed but not applied': p => p.framework.records.splice(3, 1),
  ...
  'logpoints never resumed': p => p.framework.records.at(-1).active = false,
