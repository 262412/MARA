...
ARCHIVE_PATH = (
    "libs/ktem/ktem/assets/vendor/pdfjs/pdfjs-6.1.200-dist.zip"
)
...
def test_maintainer_refresh_script_is_fixed_to_official_asset_and_hash():
    source = (REPO_ROOT / "scripts" / "download_pdfjs.sh").read_text(
        encoding="utf-8"
    )
...
def test_archive_has_exact_docker_and_precommit_allowlists():
    ...
    assert f'exclude: "^{escaped_archive_path}$"' in precommit
