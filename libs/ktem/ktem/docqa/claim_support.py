...
def _shared_claim_context(claim: str, evidence: str) -> bool:
    ...
    if not claim_tokens or not evidence_tokens:
        return False
    claim_substantive = claim_tokens - _WEAK_CONTEXT_TOKENS
    evidence_substantive = evidence_tokens - _WEAK_CONTEXT_TOKENS
    overlap = claim_substantive & evidence_substantive
    if len(overlap) >= 2:
        return True
    shortest = min(len(claim_substantive), len(evidence_substantive))
    return bool(overlap) and shortest <= 2 and len(overlap) / shortest >= 0.5
