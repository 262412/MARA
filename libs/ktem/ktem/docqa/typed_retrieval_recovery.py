...
def _recovery_document_context(request: Any) -> dict[str, str]:
    ...
    if len(selected_file_ids) > 1 or (
        ...
        return {}
    heading = _selected_document_heading(
        str(getattr(request, "selected_text", "") or "")
    )
    if not heading:
        return {}
    return {
        "kind": "selected_document_heading",
        "text": heading,
    }


def _selected_document_heading(selected_text: str) -> str:
    for raw_line in selected_text.splitlines():
        ...
        if not heading:
            return ""
        if any(prefix in heading.casefold() for prefix in _TYPED_QUERY_PREFIXES):
            return ""
        return heading[:_MAX_DOCUMENT_CONTEXT_ANCHOR_CHARS].rstrip()
    return ""
