...
class DocumentRetrievalPipeline(BaseFileIndexRetriever):
    ...
    def run(
        ...
        **kwargs,
        ...
        # rerank
        s_time = time.time()
        logger.debug("Retrieval kwargs keys: %s", tuple(retrieval_kwargs.keys()))
        docs = self.vector_retrieval(text=text, top_k=self.top_k, **retrieval_kwargs)
        logger.debug("Retrieval step took %.3fs", time.time() - s_time)
        ...
        if queries:
            try:
                ...
                logger.exception("Error retrieving additional tables")
...
class IndexDocumentPipeline(BaseFileIndexIndexing):
    ...
    def readers(self):
        ...
        logger.debug("Reader mode: %s", self.reader_mode)
    ...
    def get_pipeline(cls, user_settings, index_settings) -> BaseFileIndexIndexing:
        ...
        logger.debug("Use quick index mode: %s", use_quick_index_mode)
        obj = cls(
            embedding=embedding_models_manager[
                index_settings.get(
                    "embedding", embedding_models_manager.get_default_name()
