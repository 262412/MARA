...
from ktem.reasoning.mara_route_costing import dataset_text, is_mmdocrag_dataset
...
def _is_qasper_dataset(dataset_family: str) -> bool:
    return "qasper" in str(dataset_family or "").strip().lower()


def _explicit_graph_request(planner_route: str, planner_reason: str) -> bool:
    return (
        planner_route == "graph_global"
        and "explicit" in str(planner_reason or "").lower()
    )
