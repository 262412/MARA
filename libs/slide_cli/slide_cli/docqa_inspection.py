"""Read-only DocQA diagnostics and persisted record queries for entry adapters.

Importing this module is inert. Calling a collector bootstraps settings and may
initialize database tables through ktem.db.models; it does not create a Runtime
or initialize model managers. CLI formatting and runtime profiles live elsewhere.
"""
...
def _serialize_value(value: Any) -> Any:
    ...
    if hasattr(value, "isoformat"):
        try:
            ...
            logger.debug("Inspection date conversion failed; retaining the value.")
    return value
...
def _load_json_dict(value: Any) -> dict[str, Any]:
    ...
    if isinstance(value, str):
        try:
            ...
            logger.debug("Inspection JSON is invalid; using an empty record.")
            return {}
...
def _count_saved_sessions(
    ...
    try:
        ...
        logger.debug("Inspection session query failed; returning an issue.")
