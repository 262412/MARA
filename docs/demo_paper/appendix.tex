...
The main benchmark source for this paper is the full-system synthesis. In this workspace, the synthesis artifacts are located under:
\begin{quote}\small
\path{outputs/MARA/final_thesis_benchmark_statistical_20260705_fullsystem_postfix/09_synthesis}
\end{quote}
The key files are:
\begin{itemize}
    \item \path{statistical_synthesis_report.md}, \path{statistical_main_result_table.csv}, and \path{statistical_route_decision_summary.csv};
    \item \path{statistical_failure_latency_backend_table.csv}, \path{statistical_paired_route_delta.csv}, and \path{statistical_bootstrap_ci_by_route.csv};
    \item \path{statistical_controller_oracle_regret.csv}, \path{statistical_win_loss_tie_table.csv}, and \path{slurm_submission_jobs.tsv}.
\end{itemize}
The synthesis script is \path{build_fullsystem_synthesis.py}; the repository snapshot inspected while preparing this draft was \path{3ee655797cc16fb005b37216f9c2a32aba9463d6}. The artifact files themselves should still be treated as the authority for scores and backend metadata.
