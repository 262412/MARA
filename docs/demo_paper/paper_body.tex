...
\begin{itemize}
    ...
    \item A traceable Self-RAG-inspired controller that selects among text, visual, hybrid, CRAG-guarded, and abstention behaviours using route policies, evidence checks, workflow-cost annotations, route-switch metadata, and verification traces.
...
\end{quote}
The benchmark walkthrough starts from route manifests and produces prediction records, route metrics, and synthesis tables. This is important for a demo paper: route-aware behaviour is not only visible in the UI, but also reproducible through artifacts whose paths, scripts, backend metadata, and evaluator status are recorded in the appendix.
...
The current implementation is a structured controller, not a trained router. It uses a heuristic local planner by default and can preserve optional structured planner payloads when they are supplied. The implemented trace can record route probes, route scores, route confidences, expected quality/cost fields, skipped routes, and a cost-gate decision, but the default controller should be read as heuristic routing plus workflow cost annotations rather than as a calibrated learned route scorer. For \texttt{controller\_auto}, the runtime records evidence availability, retrieval adequacy, backend health, route-switch candidates, and verification status so that route choices can be inspected after the turn.

The route policy follows the task and evidence context. Text evidence is the default for ordinary document QA. Visual intent terms, visual modalities, or page-image availability can move the plan toward page-image routes when the backend is healthy. Structured calculation or table-like questions can justify hybrid evidence so that text, page-image, and element records can recover source values. Summary, comparison, and study-guide tasks can select graph context when the question is global rather than page-specific. For MMDocRAG-style workloads, workflow cost units and route-switch ordering make visual and hybrid routes visible as higher-cost options without claiming that the default heuristic planner has learned their expected benefit.

The trace is part of the interface contract. A controller decision records the selected route, legacy route, policy, controller mode, reason, initial route, final route, planner route, scored route, route selection policy, route scores, route confidences, expected quality, expected cost, skipped expensive routes, cost-gate decision, route probe, and route-switch candidates when available. In the current default path, some of these fields are empty unless a structured planner payload or a recovery event supplies them. The user does not need every field in the main UI, but the fields are available to the benchmark harness and to debugging workflows. This is why the paper treats the controller as a measurable system component rather than an implementation detail hidden behind a chat response.
...
3 & features $\leftarrow$ inspect task type, scope, modalities, and backend health \\
4 & \textbf{if} $p$ fixes a route \textbf{then} \\
5 & \hspace*{1em} route $\leftarrow$ requested route \\
6 & \textbf{else if} structured planner payload is available \textbf{then} \\
7 & \hspace*{1em} route $\leftarrow$ parse allowed planner route and trace fields \\
8 & \textbf{else} \\
9 & \hspace*{1em} route $\leftarrow$ heuristic route from policy, modalities, and scope \\
10 & \textbf{end if} \\
11 & plan $\leftarrow$ build workflow steps with cost units \\
12 & evidence $\leftarrow$ retrieve and normalise route evidence \\
13 & verify $\leftarrow$ check retrieval adequacy and answer support \\
14 & \textbf{if} verify fails and recovery route is available \textbf{then} \\
15 & \hspace*{1em} switch route and retry retrieval once \\
16 & \textbf{else if} verify fails \textbf{then} \\
17 & \hspace*{1em} abstain with evidence status and reason \\
18 & \textbf{end if} \\
19 & \textbf{return} answer, citations, evidence, route decision, verifier state \\
\end{tabularx}
\end{minipage}}
\caption{Controller algorithm used in the system description. Structured planner quality/cost fields are preserved when supplied; the default path uses heuristic routing, workflow-cost annotations, recovery routing, and verification traces.}
\end{figure}
