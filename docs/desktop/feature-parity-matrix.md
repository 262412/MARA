...
| ---------------------- | ----------- | ----------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| 原生选择器导入         | In progress | Main 持有原生路径；窄 IPC 无参数；sender/参数测试；Windows/Ubuntu 组合包导入链 smoke；Renderer 无路径 | Windows 10/11 产品 VM 原生选择器验收、拖放与批量选择 |
| 后台索引任务           | In progress | 真实 DocQA runtime；持久任务日志；SSE；提交 `f35a9b4` 三平台 CI 真实文本索引通过                      | 支持格式矩阵、长任务与组合包故障注入                 |
| Files 完成后刷新与删除 | In progress | 终态只刷新一次；真实 deletion coordinator；Windows/Ubuntu 非空刷新与删除 smoke                        | 批量删除和 CLI/Desktop 兼容数据验收                  |
