...
| ---------------------- | ----------- | -------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| 原生选择器导入         | In progress | Main 持有原生路径和多文件选择；拖放 File 由 Preload 解析且路径不返回 Renderer；Main 校验数量、绝对路径和扩展名       | Windows/Ubuntu 拖放 handoff 组合包、Win 10/11 真拖放 |
...
| Files 完成后刷新与删除 | In progress | 终态只刷新一次；真实 deletion coordinator；1–1,000 个 ID 的窄批量契约与可访问多选；CLI 多文件删除特征测试            | Windows/Ubuntu 批量删除组合包 smoke、Win 10/11 VM    |
