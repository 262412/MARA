...
| 会话搜索、重命名与删除 | In progress | 客户端搜索；真实 owner-scoped rename/delete；认证、幂等与窄 IPC；Windows、Ubuntu 22/24 组合包通过 | 当前包 Windows 10/11 VM；新建与固定    |

实现、测试、包体测量与剩余风险见
[Gate 3 文件索引纵向切片证据](gate-3-file-indexing-evidence.md)和
[Gate 3 会话详情纵向切片证据](gate-3-session-detail-evidence.md)、
[Gate 3 会话管理纵向切片证据](gate-3-session-management-evidence.md)。
