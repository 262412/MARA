...
\title{LLM Agents for Multimodal Applications: MARA, a Local Document Question-Answering Workbench with a Self-RAG-Inspired Controller}
...
\begin{abstract}
MARA (Multimodal Agentic Retrieval and Answering) is a proposed local-first document question-answering workbench for mixed academic and technical documents. Its main technical feature is a Self-RAG-inspired controller: a program component that decides how the system should answer each question. The system will provide document upload, indexing, source selection, answers with citations, page and evidence preview, knowledge-map context, notes, learning materials and a shared Web/CLI workflow. Instead of using one fixed retrieval-augmented generation (RAG) pipeline, MARA can answer directly, retrieve text, inspect page images, use document elements such as tables or figures, use graph context, combine several routes, check evidence, retry, revise or abstain. The analytical component will use technical tests, demonstration tasks and optional small route comparisons to support, rather than dominate, the project.
\end{abstract}
...
Data category: A0/A1 for public, synthetic or self-provided non-sensitive project documents. No human participants are planned. Evaluation will use technical tests, demonstration tasks and optional manually annotated non-sensitive evidence labels. I will follow the School ethical guidance, use permitted data only, avoid confidential personal data, and confirm any later change with my supervisor before it begins.

\section{Project Description}

MARA will help users search, compare and understand mixed document collections through a web workbench and command-line workflow. The project is framed first as a usable MSc project prototype: users can add documents, index them locally, ask questions, inspect cited evidence, select sources, view knowledge-map context, and create notes or learning materials. Its main technical contribution is a controller that combines several ways of answering: text retrieval, page-image evidence, document elements and graph context. The controller receives a \texttt{DocQARequest}, chooses one or more answer routes, retrieves evidence, checks evidence quality, generates or rejects an answer, and returns a \texttt{DocQAResponse} with citations, evidence and trace metadata.
...
\begin{itemize}
  ...
  \item To support document upload, indexing, source selection, citation-backed answers, page/evidence preview, knowledge graph context, notes and learning materials.
  \item To design a Self-RAG-inspired controller that can select and coordinate direct answering, text RAG, page-image RAG, element RAG, GraphRAG-style context, hybrid retrieval and abstention.
  \item To improve transparency and reliability through structured evidence records, page/element citations, retrieval quality checks, optional claim verification and traceable route decisions.
  \item To validate the system through automated tests, integration scenarios, demonstration tasks and, where time permits, a small technical comparison of answer routes.
\end{itemize}

\textbf{Scope note:} in this proposal, ``essential'' means a usable workbench, a stable controller interface, simplified but working answer routes, and enough technical evaluation to explain system behaviour. It does not mean a production-level reproduction of Self-RAG, CRAG, ColPali, GraphRAG or MMDocRAG. Full visual-language backends, full GraphRAG engineering, trainable routing and large benchmark studies are configurable or scoped extensions.
...
\begin{itemize}
  ...
  \item Users can select document sources, inspect cited pages or evidence snippets, and continue conversations with persisted local state.
  \item Users can use knowledge-map context, notes and source-grounded learning materials where the available evidence supports them.
  ...
  \item The prototype implements usable versions of \texttt{text\_rag}, controller-auto routing and at least one multimodal or structured-evidence route, while clearly labelling any smoke or unavailable backend.
  \item The indexing layer stores text chunks, page-image records, element records and lightweight graph records with links back to the original sources.
  \item The controller selects routes, records reasons, respects allowed routes, and handles invalid route decisions through a safe fallback or abstention.
  \item Lightweight CRAG-style retrieval quality checks and optional claim verification help decide whether to generate, retry, revise or abstain.
  \item Answers include citations, evidence metadata and a compact trace of controller decisions that can be inspected in the UI, CLI and optional evaluation output.
  \item Automated tests cover indexing, retrieval, citation handling, controller decisions, route executors, evidence schemas, Web/CLI contracts and important UI services.
\end{itemize}
...
\begin{itemize}
  ...
  \item A small technical comparison of answer routes, such as fixed text RAG, controller-auto routing, hybrid retrieval and evidence-checked answering, when time and system quality permit.
...
\midrule
New or extended work & Controller contracts, route/executor registry, structured evidence schema, scoped page-image and element indexing, lightweight GraphRAG route, CRAG-style evidence checker, claim verifier, optional route comparison adapters, shared Web/CLI runtime and inspectable controller trace. \\
\bottomrule
\end{tabularx}
\end{center}
...
\begin{enumerate}
  ...
  \item Run claim verification; unsupported or insufficient evidence triggers one revision, retry or abstention.
...
If a small technical evaluation is included, it will use public or synthetic multimodal documents and manually prepared question-answer pairs. Each item may include the question, expected answer route, target document type, expected answer, accepted evidence references, and source identifiers such as file name, page, slide, table, figure, bounding box, element ID or graph community. System outputs will also record backend metadata so that local smoke backends are not confused with configured ColPali, VLM or LLM-planner backends. No private scraping, personal records, confidential documents or participant feedback are required.
...
Project evaluation will focus first on whether MARA satisfies the implemented requirements through automated tests, integration scenarios and demonstration tasks. The analytical component may add a small comparison of answer routes, including direct baseline, fixed text RAG, at least one multimodal route, controller-auto routing, hybrid retrieval and CRAG-guarded execution. Other configured routes, such as page-image, element or graph-inspired routes, will be reported where their backends are available.

The optional analytical metrics will be inspired by several evaluation traditions. ALCE-style signals may report whether citations point to the right sources. MMDocRAG-style signals may report whether the system found the right page, element, image or cross-page evidence. RAGTruth-style signals may report unsupported claims, contradictions and whether abstention was correct. The evaluation may also report routing accuracy, retrieval success, latency, backend type, cost where relevant, and qualitative error categories. These metrics will be adapted at prototype scale rather than treated as full external benchmark reproductions.

\section{Project Ethics and Human Participants}

No human participants or beta testers are planned. Evaluation will use technical tests, demonstration tasks and optional analytical fixtures built from public, synthetic or self-provided non-sensitive documents. No recruitment, interview, survey, observation or personal feedback data will be collected. If I later add a user study, I will update the ethics statement and check with my supervisor first.
...
\midrule
Practical and analytical skills & Python development, runtime modularisation, retrieval, multimodal indexing, controller design, Web/CLI integration, testing and optional analytical evaluation. \\
...
Synthesis and evaluation & Literature on RAG, Self-RAG, adaptive routing, multimodal DocRAG, ColPali, GraphRAG, CRAG, document workbench design and citation/hallucination evaluation. \\
...
Self-management & Boundary freeze, version control, route milestones, contract tests, demonstration planning, optional evaluation design, risk tracking and dissertation writing time. \\
...
The interface will be a multimodal document workbench: corpus browser on the left, page/evidence preview in the centre, and route-aware answering, citations, verification and graph context on the right.
...
The left panel manages sources. The middle shows selected page, image, text and element evidence. The right panel exposes the route policy, answer, citations, controller trace, verification result and graph context. Controller and evaluation controls will be kept compact so the Web UI remains a product prototype while the CLI provides reproducible workflows and optional technical analysis.
...
Analytical scope becomes too broad & Keep the technical comparison optional, start with small synthetic fixtures, and prioritise the usable workbench & Medium & High \\
