...
\centering
\vspace*{2.5cm}
{\Large\bfseries MARA: A Local Retrieval-Augmented Document QA and Knowledge Exploration System\par}
\vspace{2cm}
{\large Name: [Chenghao Zhang]\par}
\vspace{0.3cm}
{\large Student ID: [201844864]\par}
\vfill
\begin{minipage}{0.9\textwidth}
\textbf{Statement of Ethical Compliance.}
...
\begin{itemize}
  \item To develop a usable local RAG application for document question answering with clear citation and preview support.
  \item To improve transparency by linking generated answers to retrieved evidence, page previews and a conversation knowledge graph.
  \item To evaluate whether the system meets practical user needs for searching, summarising and exploring multi-format documents.
\end{itemize}

\textbf{Essential requirements:}
\begin{itemize}
  \item Users can upload and index supported document formats, including PDF, Office, text and Markdown files.
  \item Users can ask questions over all indexed documents or selected files.
  \item The system retrieves relevant document chunks and generates answers with citations.
  \item Users can open cited sources in a preview panel and inspect page-level evidence.
  \item Conversations and indexes are persisted locally so the web UI and CLI can share the same runtime state.
  \item The application includes a knowledge graph or mind-map style view that helps users explore document topics and ask follow-up questions.
  \item The project includes automated tests for indexing, retrieval, citation handling, CLI behaviour and important UI services.
\end{itemize}
...
\begin{itemize}
  ...
  \item A small benchmark set for comparing retrieval quality across example questions.
  \item A polished light and dark UI theme suitable for long reading sessions.
\end{itemize}

\section{Key Literature and Background Reading}

The technical basis of the project is retrieval-augmented generation. Lewis et al. describe RAG as a method that combines a parametric language model with a non-parametric external knowledge source, allowing generated answers to be grounded in retrieved passages rather than relying only on model memory \cite{lewis2020rag}. This directly supports MARA's design: documents are first indexed, then retrieved evidence is supplied to the answer-generation pipeline.
...
\begin{enumerate}
  ...
  \item Extract text and metadata from each file.
  \item Split documents into chunks and store them in a document store.
  \item Generate embeddings and store them in a vector store.
  \item Retrieve relevant chunks for a user question.
  \item Pass the retrieved context to the language model.
  \item Display the answer, citations, retrieved evidence and preview links.
